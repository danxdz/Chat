...
function viewInvitations() {
    ...
    const inviteList = Object.values(invitations)
        ...
        .map(inv => {
            ...
            const status = inv.used ? '✅ Used' : (isExpired ? '⏰ Expired' : '🟢 Active');
            const date = new Date(inv.createdAt).toLocaleDateString();
            const usedInfo = inv.used && inv.usedBy ? ` by ${inv.usedBy}` : '';
...
async function initSodium() {
    ...
    try {
        // Wait for sodium to be available (from CDN include)
        let attempts = 0;
        const maxAttempts = 100; // up to ~10s
        while ((!window.sodium && !window.sodiumReady) && attempts < maxAttempts) {
            await new Promise(resolve => setTimeout(resolve, 100));
            attempts++;
        }
        if (!window.sodium) {
            throw new Error('Sodium library not found on window');
        }
        // Await readiness
...
// Removed fallback and network check functions to simplify initialization with CDN sodium
// function tryAlternativeInit() { /* removed */ }
// function initWebCryptoFallback() { /* removed */ }
// function checkNetworkConnectivity() { /* removed */ }
// function loadFallbackLibraries() { /* removed */ }
