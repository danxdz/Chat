...
/**
 * Generate Diffie-Hellman shared secret for E2E encryption
 * @param {string} myPrivateKey - My private key
 * @param {string} theirPublicKey - Their public key
 * @returns {Promise<string>} - Shared secret
 */
export const generateSharedSecret = async (myPrivateKey, theirPublicKey) => {
  if (!window.Gun?.SEA) {
    throw new Error('Gun.SEA not available');
  }
  
  try {
    // Use Gun.SEA.secret for ECDH key agreement
    const sharedSecret = await window.Gun.SEA.secret(theirPublicKey, myPrivateKey);
    return sharedSecret;
  } catch (error) {
...
/**
 * Validate invite token signature
 * @param {object} inviteData - Invite data object
 * @param {string} signature - Signature to verify
 * @param {string} publicKey - Public key of signer
 * @returns {Promise<boolean>} - True if signature is valid
 */
export const verifyInviteSignature = async (inviteData, signature, publicKey) => {
  if (!window.Gun?.SEA) {
    throw new Error('Gun.SEA not available');
  }
  ...
  try {
    ...
    const isValid = await window.Gun.SEA.verify(signature, message, publicKey);
...
/**
 * Create signed invite token
 * @param {object} inviteData - Invite data
 * @param {string} privateKey - Private key for signing
 * @returns {Promise<string>} - Signed invite token
 */
export const createSignedInvite = async (inviteData, privateKey) => {
  if (!window.Gun?.SEA) {
    throw new Error('Gun.SEA not available');
  }
  ...
  try {
    ...
    const signature = await window.Gun.SEA.sign(message, privateKey);
    ...
    return btoa(JSON.stringify(signedInvite));
  } catch (error) {
    console.error('Failed to create signed invite:', error);
    throw error;
  }
};
